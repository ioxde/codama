 ...
 *   - `pdaValueNode`: return `null` when a present `programId` visits
 *     away, rather than silently deriving under the declared program.
...
export function identityVisitor<TNodeKind extends NodeKind = NodeKind>(
    ...
    if (keys.includes('pdaValueNode' as TNodeKind)) {
        // ioxde fork: a `programId` that visits away must not decay to `undefined` — that silently re-derives
        // the PDA under a different program, with no error. Cascade null so resolution fails ACCOUNT_MISSING.
        // `resolveRuntimeProgramId` in dynamic-address-resolution is lenient on a missing value; don't harmonize.
        overrides.visitPdaValue = function visitPdaValue(node, { next }) {
            const visited = next(node);
            if (visited === null || visited.kind !== 'pdaValueNode') return visited;
            if (node.programId && visited.programId === undefined) return null;
            return visited;
        };
    }
